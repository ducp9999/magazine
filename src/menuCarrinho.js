...
function incrementarQauntidadeProduto(idProduto) {
    idsProdutoCarrinhoComQuantidade[idProduto] ++;
    salvarLocalStorage('carrinho', idsProdutoCarrinhoComQuantidade);
    atualizarPrecoCarrinho();
    atualizarInformacaoQauntidade(idProduto);
}
...
function decrementarQauntidadeProduto(idProduto) {
...
function atualizarInformacaoQauntidade(idProduto) {
...
function desenharProdutoNoCarrinho(idProduto) {
    ...
    const cartaoProdutoCarrinho = `<button id="remover-item-${produto.id}" class="absolute top-0 right-2"><i class="fa-solid fa-circle-xmark text-slate-500 hover:text-slate-800"></i></button>
        ...
        <div class="p-2 flex flex-col justfy-between">
    ...
    document.getElementById(`decrementar-produto-${produto.id}`).addEventListener('click', () => decrementarQauntidadeProduto(produto.id));
    document.getElementById(`incrementar-produto-${produto.id}`).addEventListener('click', () => incrementarQauntidadeProduto(produto.id));
...
export function adicionarAoCarrinho(idProduto) {
    if (idProduto in idsProdutoCarrinhoComQuantidade) {
        incrementarQauntidadeProduto(idProduto);
        return;
    }
